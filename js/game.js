var game = {
	...
	setHandlers: function () {
		...
		$(window)
			...
			.on("hashchange", function () {
				...
				$("#tweet iframe").remove();
				var html =
					'<a href="https://twitter.com/share" class="twitter-share-button"{count} data-url="https://cssgridgarden.com" data-via="thomashpark" data-text="I just beat Grid Garden, a game for learning CSS grid layout!">Tweet</a> ' +
	...
	loadLevel: function (level) {
		...
		for (var i = 0; i < string.length; i++) {
			...
			var plant = $("<div/>")
				.addClass("plant " + color)
				.data("color", color);
			var treatment = $("<div/>")
				.addClass("treatment " + color)
				.data("color", color);
	...
	check: function () {
		...
		$(".treatment").each(function () {
			...
			position.width = Math.floor(
				parseFloat(window.getComputedStyle(this).width),
			);
			position.height = Math.floor(
				parseFloat(window.getComputedStyle(this).height),
			);
...
$(document).ready(function () {
	...
	if (!("gridArea" in d)) {
		var warning =
			messages.warningUnsupported[game.language] ||
			messages.warningUnsupported.en;
